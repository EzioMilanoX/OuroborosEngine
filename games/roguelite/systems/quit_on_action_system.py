...
class QuitOnActionSystem(ISystem):
    ...
    `data/input_bindings/default_keyboard.json`) e pressionada.

    So roda enquanto `GameplayScene` estiver no topo da pilha -- a
    `EndScene` (fim de jogo) checa a mesma acao diretamente dentro do
    proprio `update()`, sem depender deste sistema (mesmo idioma de
    `PauseScene`).
    """
